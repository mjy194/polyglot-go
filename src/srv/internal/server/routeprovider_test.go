...
func TestRouteProviderEmptyModeIsPassthrough(t *testing.T) {
	...
	p, ok := srv.routeProvider(context.Background(), passthrough.ProtocolAnthropic)
	...
	if p, ok := srv.routeProvider(context.Background(), passthrough.ProtocolGemini); ok || p != nil {
...
func TestRouteProviderPassthroughMatchesByType(t *testing.T) {
	...
	for _, proto := range []string{passthrough.ProtocolOpenAI, passthrough.ProtocolResponses} {
		p, ok := srv.routeProvider(context.Background(), proto)
	...
	if p, ok := srv.routeProvider(context.Background(), passthrough.ProtocolAnthropic); ok || p != nil {
...
func TestRouteProviderPassthroughOverridesAdapter(t *testing.T) {
	...
	p, ok = srv.routeProvider(context.Background(), passthrough.ProtocolOpenAI)
